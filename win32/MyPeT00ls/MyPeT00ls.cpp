...
{									
	...
	case  WM_COMMAND :								
		...
		case  IDC_BUTTON_Addres :
			{					
				DialogBox(gHInstance,MAKEINTRESOURCE(IDD_DIALOG_AddWater),hwndDlg, DialogProc );
				return TRUE;
			}
		...
		case  IDC_BUTTON_SRC :
			{					
				...
				setOenFile.nMaxFile = MAX_PATH;
				GetOpenFileName(&setOenFile);
				SendDlgItemMessage(hwndDlg,IDC_EDIT_SRC,WM_SETTEXT,0,(DWORD)szFileName);
		...
		case  IDC_BUTTON_OUT :
			{					
				...
				setOenFile.nMaxFile = MAX_PATH;
				GetOpenFileName(&setOenFile);
				SendDlgItemMessage(hwndDlg,IDC_EDIT_OUT,WM_SETTEXT,0,(DWORD)szFileName);

				return TRUE;
			}
		case   IDC_BUTTON_ADDWATER:												
			{
				char SrcPath[120],OutPath[120];
				GetDlgItemText(hwndDlg,IDC_EDIT_SRC,SrcPath,120);
				GetDlgItemText(hwndDlg,IDC_EDIT_OUT,OutPath,120);
				
				if(strlen(SrcPath) < 5 && strlen(OutPath) < 5){
					MessageBox(0,"源文件或者壳文件不能为空",0,0);
					return 0;
				}
				//读取源文件
				LPSTR FILEPATH = SrcPath;
				LPVOID SrcFileBuffer = NULL;
				DWORD SrcFileSize =0;
				SrcFileSize = ReadPeFile(SrcPath,&SrcFileBuffer);
				//加密
				XorEncryptAAA((char*)SrcFileBuffer,SrcFileSize);
				//读取壳文件
				LPSTR OUTFILEPATH = OutPath;
				LPVOID OutFileBuffer = NULL;
				ReadPeFile(OUTFILEPATH,&OutFileBuffer);
				//给壳文件新增一个节
				BOOL Falg = SAddNewSection(OutFileBuffer,SrcFileBuffer,SrcFileSize);
				if (Falg)
				{
					MessageBoxA(NULL,TEXT("加壳成功"),NULL,NULL);
				}
				return TRUE;
			}
