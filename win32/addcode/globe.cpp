    ...
    while (true) {
		...
		if (ReloCation->VirtualAddress != 0 && ReloCation->SizeOfBlock !=0)
		{
			...
			for (int i =0;i<num-1;i++)  //循环表
			{
				...
				if (*offset >= 0x3000)
				{
					DWORD Foa =  RvaToFileOffset(pFileBuffer,(ReloCation->VirtualAddress + ( *offset-0x3000)));
					printf("第%x项\t文件偏移%x\t type:3\n",i,Foa);
				}
			}
...
DWORD printBoundImportTable(LPVOID pFileBuffer){
	...
	PIMAGE_BOUND_IMPORT_DESCRIPTOR BindImportExtable = (PIMAGE_BOUND_IMPORT_DESCRIPTOR)((char*)pFileBuffer + RvaToFileOffset(pFileBuffer,pOptionHeader->DataDirectory[11].VirtualAddress));
	
	PIMAGE_BOUND_IMPORT_DESCRIPTOR boundDir = BindImportExtable;

	
	while(BindImportExtable->TimeDateStamp){
		printf("====%s=======\n",(DWORD)BindImportExtable+BindImportExtable->OffsetModuleName);
		printf("时间戳%d\n",BindImportExtable->TimeDateStamp);
		printf("依赖dll个数%d\n",BindImportExtable->NumberOfModuleForwarderRefs);
		int i  = BindImportExtable->NumberOfModuleForwarderRefs;	
		if (i>0)
		{
			PIMAGE_BOUND_FORWARDER_REF bondRef = (PIMAGE_BOUND_FORWARDER_REF) ((DWORD) BindImportExtable + 8);
            printf("***********依赖dll***********\n");
            printf("依赖dll\t时间戳\n");
            for(int j=0;j<i;j++){
            ...
            BindImportExtable = BindImportExtable + (i+1);
		} 
		else
		{
			BindImportExtable++;
...
DWORD MoveRelocation(LPVOID pFilebuff){
	...
	//移动重定位表
	tempSecAddr = pNewSecAddr;
    while (true) {
          
		//先判断重定位表是否结束 0代表已经结束
		if (ReloCation->VirtualAddress != 0 && ReloCation->SizeOfBlock !=0)
		{
			//cnt += ReloCation->SizeOfBlock;
			memcpy(tempSecAddr, ReloCation, ReloCation->SizeOfBlock);
			tempSecAddr = (PVOID)((DWORD)tempSecAddr + ReloCation->SizeOfBlock); 
			ReloCation = (_IMAGE_BASE_RELOCATION*)((char*)ReloCation + ReloCation->SizeOfBlock);
	...
	//将重定位表的地址指向新节表的地址 新节表的地址=文件偏移转换为内存偏移
    pOptionHeader->DataDirectory[5].VirtualAddress = FoaToImageOffset(NewBuffer,pNewSec->PointerToRawData);
	
	
	//修改imagesbase
	int increment = 0x100000;
	pOptionHeader->ImageBase = pOptionHeader->ImageBase + increment;
	printf("ImageBase:%x", pOptionHeader->ImageBase);
	
	//重新定位重定位表
	PIMAGE_BASE_RELOCATION pRelocationTable = (_IMAGE_BASE_RELOCATION*)((char*)NewBuffer + RvaToFileOffset(NewBuffer,pOptionHeader->DataDirectory[5].VirtualAddress));
	DWORD RVA_Data;
	...
	while (pRelocationTable->VirtualAddress && pRelocationTable->SizeOfBlock)
	{
		...
		{
			WORD* offset = (WORD*)((char*)pRelocationTable+8+2*j); //每个数据项的地址
			if (*offset >= 0x3000)
			{
				//低12位加上x就是要修改的地方
				
				DWORD Foa = (DWORD)NewBuffer + RvaToFileOffset(NewBuffer,pRelocationTable->VirtualAddress + ( *offset-0x3000));	//变为FOA再变为绝对地址才能找到真正的值
				*((PDWORD)Foa) += increment;
			}
	...
	//修复重定位表
	 while (true) {
		...
		//先判断重定位表是否结束 0代表已经结束
		if (pRelocationDirectory->VirtualAddress != 0 && pRelocationDirectory->SizeOfBlock !=0)
		{
			printf("**********************************\n");
			printf("%x\n",pRelocationDirectory);
			//获取一共有多少块数据 数量 = 重定位表大小 -  8 (VirtualAddress大小 + SizeOfBlock 一共占了8字节) / 2 (因为每个块站2个字节的大小)
			int num = (pRelocationDirectory->SizeOfBlock - 8) / 2;
			for (int i =0;i<num;i++)  //循环表
			{
			
				//获取表的地址
			     WORD* offset = (WORD*)((char*)pRelocationDirectory+8+2*i);
				//判断高位是否 =3 高位是否大于= 0011
				if (*offset >= 0x3000)
				{
					//低12位加上x就是要修改的地方

					DWORD Foa =(DWORD)NewBuffer + RvaToFileOffset(NewBuffer,(pRelocationDirectory->VirtualAddress + ( *offset-0x3000)));
					//printf("第%x项\t文件偏移%x\t type:3\n",i,Foa);
					//printf("第%x项\t地址:%X\t偏移:%d\n", pRelocationDirectory->VirtualAddress, *offset-0x3000);
					*((PDWORD)Foa) += 1000;			//修复这个值		
				}
			}
					
			pRelocationDirectory = (_IMAGE_BASE_RELOCATION*)((char*)pRelocationDirectory + pRelocationDirectory->SizeOfBlock);
	...
	*/
	...
	FILE* fp = fopen("C://project/newdll.dll","wb+");
