...
DWORD PrintImportTable(LPVOID pImageBuffer){
	...
	PIMAGE_IMPORT_DESCRIPTOR ImportExtable = (PIMAGE_IMPORT_DESCRIPTOR)((char*)pImageBuffer + pOptionHeader->DataDirectory[1].VirtualAddress);
	
	while(ImportExtable->OriginalFirstThunk !=0 && ImportExtable->FirstThunk !=0){

		char *DllName =(char*)pImageBuffer + ImportExtable->Name;
		...
		DWORD* pThunkData_INT = (DWORD*)((char*)pImageBuffer + ImportExtable->OriginalFirstThunk);
		DWORD*	pThunkData_IAT = (PDWORD)(((PBYTE)pImageBuffer + ImportExtable->FirstThunk));
		while(*pThunkData_INT){
			...
			if (*pThunkData_INT & 0x80000000)
			{
				...
				PIMAGE_IMPORT_BY_NAME FirstThunkNames = (_IMAGE_IMPORT_BY_NAME*)((char*)pImageBuffer + *pThunkData_INT);
